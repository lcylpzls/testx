...
- README / docs 与当前代码同步：移除 testx/logx 旧表述与过期规划文档。
...
- 删除 `testx/logx` 子包（全家族零引用），testx 面向所有包一视同仁，依赖仅剩 errx；
- 补齐比较断言非数值分支与 nil 分支测试。
...
- 依赖升级：errx v1.5.7、logx v1.3.4（go get -u -t all）。
